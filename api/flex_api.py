...
# OpenAI client
client = AsyncOpenAI(
    api_key=os.getenv("OPENAI_API_KEY"), http_client=httpx.AsyncClient()
)
...
def determine_experiment_type(prompt: str) -> str:
    ...
    if "serial dilution" in prompt_lower:
        ...
        term in prompt_lower for term in ["enzyme", "assay", "substrate", "kinetic"]
    ):
        return "enzyme_assay"
    else:
        return "generic"
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as http_client:
            try:
                ...
                opentrons_status = (
                    "connected"
                    if opentrons_response.status_code == 200
                    else "disconnected"
                )
            except:
        ...
        return {
            ...
            "services": {
                "openai": (
                    "connected" if os.getenv("OPENAI_API_KEY") else "not_configured"
                ),
...
async def generate_protocol(req: ExperimentRequest):
    ...
    try:
        ...
        path = save_protocol(
            full_protocol, filename=f"generated_protocol_{experiment_type}.py"
        )
...
async def validate_protocol(req: ProtocolValidationRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Protocol validation failed: {str(e)}"
        )
...
async def send_to_flex(req: FlexRunRequest):
    ...
    try:
        if not os.path.exists(filepath):
            raise HTTPException(
                status_code=400, detail=f"Protocol file not found: {filepath}"
            )
        ...
        async with httpx.AsyncClient(timeout=30.0) as http_client:
            upload_response = await http_client.post(
                f"{BASE_URL}/protocols", files=files, headers=headers
            )
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Internal error while sending to Flex: {str(e)}"
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as http_client:
            response = await http_client.get(
                f"{BASE_URL}/runs/{run_id}", headers={"opentrons-version": "2"}
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting run status: {str(e)}"
        )
...
async def list_protocols():
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as http_client:
            response = await http_client.get(
                f"{BASE_URL}/protocols", headers={"opentrons-version": "2"}
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error listing protocols: {str(e)}"
        )
...
async def startup_event():
    ...
    print(
        f"🤖 OpenAI API Key configured: {'✅' if os.getenv('OPENAI_API_KEY') else '❌'}"
    )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as http_client:
            ...
            print(
                f"🔬 Opentrons Flex connection: {'✅ Connected' if response.status_code == 200 else '❌ Failed'}"
            )
    except:
